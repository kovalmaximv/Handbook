...
## Модульные тесты
Правильное следование TDD методологии генерирует большой объем тестов. Такой объем тестов может создать проблемы, если
не следить в них за качеством кода.

Тесты на скорую руку равносильны отсутствию тестов. Дело в том, что тесты должны изменяться по мере развития кода 
продукта. Чем хуже написаны тесты, тем сложнее их будет модифицировать вместе с кодом. Отказаться от тестов тоже нельзя,
они необходимы программистам, чтобы протестировать изменения в коде. Так же без тестов рефакторинг будет сложно 
реализуем. При наличии тестов можно свободно вносить изменения даже в далеко не идеальную архитектуру.

В тестах необходимо придерживаться всех тех же правил чистого кода, которые вы используете при написании кода продукта.

В одном тесте стоит проверять одну концепцию. Мы не хотим, чтобы длинные тестовые функции делали несколько разношерстных
проверок одна за другой.

## Классы
Иногда для переменной или функции необходимо понижать инкапсуляцию, чтобы их можно было протестировать. Это определенно
стоит того, поскольку тесты важны, но сначала стоит подумать, а есть ли другие способы решения проблемы. Понижение 
инкапсуляции это последняя мера.

Класс должен быть максимально компактным. По принципу единой ответственности (SRP) класс должен иметь одну причину для 
изменения. 

Имя класса должно описывать его ответственность. Если для класса не удается
подобрать четкое и короткое имя, то вероятно класс слишком велик. Следующие слова-маркеры сигнализируют о том, что у 
класса много ответственности и следует его поделить: processor, manager, super, etc.

Многие разработчики боятся, что если следовать этим правилам, то система будет иметь слишком много маленьких частей и в 
ней будет сложно разбираться. Это не так, ведь количество кода, в обеих версиях системы одинаковое, но легче разбираться
будет в системе с большим количеством маленьких компонентов. [Вот тут я бы поспорил. Порой легче переварить один 
компонент средней величины, чем разбираться в зоопарке маленьких классов. Нужно придерживаться золотой середины.]

Каждый класс можно охарактеризовать величиной связности. В классе с высокой связностью каждое поле используется в каждом 
методе. При составлении классов нужно стремиться к высокой связности. Если связность низкая, это сигнализирует о том, что 
класс можно разделить на классы поменьше.

По возможности следует отказываться от аргументов в методах класса и делать эти аргументы полями класса. Особенно если
аргументы передаются из метода в метод.

## Системы
В системах необходимо разделять фазы инициализации, когда создаются необходимые объекты, с фазой выполнения, 
управляющей созданными объектами. Способов разделения несколько: 
1) Перемещение конструирования в main функцию. После конструирования main функция передает объекты приложению.
2) Фабрики: в некоторых случаях объект нужно создавать не при старте приложения, а по требованию. В таки случаях 
используются фабрики.
3) Dependency Injection - IOC контейнеры наподобие того, что есть в Spring. Класс не принимает решение о внедрении 
зависимостей и остается пассивным. Вместо этого он представляет set методы или аргументы конструктора, а контейнер 
создает необходимые объекты и использует эти методы или аргументы, чтобы внедрять зависимости. 

Оптимальная архитектура системы состоит модульных областей, каждая из которых реализует свою зону ответственности. 
Модули интегрируются между собой посредством аспектов, минимально вмешивающихся в их работу. Трудно переоценить важность
разделения ответственности посредством AOP.

Например, в Spring бизнес логика записывает посредством POJO объектов. Такие объекты полностью сосредоточены на своей
предметной области, а значит они проще в восприятии и легче в тестировании. Вся необходимая структура приложения 
реализуется посредством объявления файлов конфигурации. В этих конфигурациях вы объявляете объекты аспектов, а Spring 
берет на себя их механику взаимодействия с вашими POJO объектами.

Принятие системных решений лучше всего откладывать до последнего момента. Дело не в лени или безответственности. Таким 
образом мы хотим накопить максимум информации и исходя из этого принять правильное решение. 

## Многопоточность
С многопоточностью связан целый ряд мифов и неверных представлений
- **Многопоточность всегда повышает быстродействие**: только при большом времени ожидания потоков, которое может быть 
использовано более эффективно. Не получится ускорить последовательные вычисления с помощью многопоточности
- **Написание многопоточного кода не изменяет архитектуру программы**: архитектура многопоточного кода существенно 
отличается от однопоточного кода.

А вот точные утверждения:
- Многопоточность сопряжена с дополнительными затратами, как в отношении производительности, так и написании 
...
- Многопоточность часто требует фундаментельных изменений в стратегии проектирования

Защита от ошибок многопоточности:
- Отделяйте код, относящийся к реализации многопоточности, от остального кода
- Жестко ограничьте доступ ко всем общим данным. Для этого либо используйте копии данных, либо проектируйте 
многопоточный код так, чтобы каждый поток существовал в своем замкнутом пространстве и не использовал данные совместно 
с другими потоками
- Изучайте стандартные Java пакеты для работы с многопоточностью
- Блоки synchronized должны иметь минимальные размеры

Для тестирования многопоточного кода пишите тесты, направленные на выявление существующих проблем (без акцентирования
на многопоточности). Затем часто выполняйте эти тесты для разных программных/системных конфигураций и уровней нагрузки. 
Тестируйте программу с количеством потоков превышающих количество процессоров. Можно в случайных местах программы 
написать wait(), sleep(), yield(), чтобы попытаться отловить как можно больше проблем связанных с многопоточностью 
(главное после не забыть убрать это из кода). Если при выполнении теста произошла ошибка, обязательно найдите причину. 

# Мнение
- Чтобы научиться писать чистый код, нужно читать много кода и иметь большую насмотренность
- Если слепо следовать всем советам из книги, то легко можно уйти в крайность и код будет далеко не чистым (при этом 
Боб настаивает, что он истина в последней инстанции)
- Одни и те же советы повторяются по несколько раз за всю книгу
- Секретную истину ожидать от книги явно не стоит