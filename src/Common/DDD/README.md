...
целое. Все сущности доступны только через корень агрегата. Customer, рассмотренный нами выше, представляет собой 
агрегат. Он аккумулирует в себе сущности Customer и Order, а так же value objects email и status. Другие элементы
домена не могут напрямую общаться с внутренностями агрегата, только через него (его методы). Мы не может выполнить 
отдельный запрос в репозиторий и получить адрес клиента по id или сумму его третьего заказа. Нам необходимо запросить 
весь агрегат.
