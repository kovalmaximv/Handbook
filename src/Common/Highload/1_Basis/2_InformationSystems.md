...
2. [Реляционная модель vs Документоориентированная модель](#реляционная-модель-vs-документоориентированная-модель)  
...
4. [Графоподобные модели данных](#графоподобные-модели-данных)  
   4.1 [Граф свойств и язык запросов CYPHER](#граф-свойств-и-язык-запросов-cypher)  
   4.1 [Хранилище тройных кортежей и SPARQL](#хранилище-тройных-кортежей-и-sparql)  
5. [Итоги](#итоги)

# Модели данных и языки запросов
Под моделями данных здесь подразумеваются различные СУБД (реляционные и не реляционные). 

Так же стоит вспомнить определения императивных и декларативных языков. Декларативные языки (например SQL)
...
В хранилище тройных кортежей вся информация хранится в виде трехкомпонентных высказываний: 
_(субъект, предикат, объект)_. Например `(Lucy, live, Idaho)`, что значит Люси живет в Айдахо.

Субъект тройного предиката эквивалентен вершине графа, объект представляет одну из двух вещей:
1) Значение просто типа данных (строка, число). В этом случае предикат и объект представляют собой свойство вершины. 
Например `(Lucy, age, 33)`
2) Другую вершину графа. В этом случае предикат представляет собой ребро графа, субъект начальную вершину, а объект 
конечную. Например `(Lucy, marriedTo, Alain)`.

Распространенная реализация тройных кортежей является модель данных RDF или Turtle. Turtle более удобочитаем, для RDF
есть свой язык запросов. Существуют утилиты, которые умеют переводить из одной модели данных в другую.

SPARQL - язык запросов для хранилищ тройных кортежей реализованных в модели RDF. Попробуем реализовать тот же поиск 
людей эмигрировавших из США в Европу (из предыдущей главы), но уже на языке SPARQL.
...
# Итоги
Исторически сначала данные представлялись в виде одного большого дерева (иерархическая модель данных). Такой вариант
плохо работал со связями many-to-many и для решения этой проблемы была придумана реляционная модель. Чуть позже 
разработчики обнаружили, что она не очень хорошо подходит для некоторых приложений. Новые нереляционные модели данных
получили общее название NoSQL и делятся на две основные разновидности:

1) Документоориентированные БД - подходят когда данные представляют собой независимые документы и связи между 
документами редки.
2) Графовые БД - противоположны документоориентированным, подходят когда любые данные могут быть связаны.

Все три модели (реляционная, документная и графовая) в настоящее время широко используются. Одна из общих черт 
документных и графовых моделей - отсутствие обязательной схемы для хранения данных. 