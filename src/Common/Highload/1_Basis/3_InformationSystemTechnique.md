...
1. [Введение](#подсистемы-хранения-и-извлечения-данных)
2. [Базовые структуры данных БД](#базовые-структуры-данных-бд)  
   2.1. [Хеш индексы](#хеш-индексы) 
   2.2. [SS таблицы и LSM деревья](#ss-таблицы-и-lsm-деревья)  
   2.3. [B деревья](#b-деревья)  
   2.4. [Сравнения B и LSM деревьев](#сравнения-b-и-lsm-деревьев)  
3. [Аналитические базы данных](#аналитические-базы-данных)  
   3.1. [Столбцовое хранилище](#столбцовое-хранилище)
4. [Резюме](#резюме)
...
| Свойство                    | OLTP                                                                        | OLAP                                              |
|-----------------------------|-----------------------------------------------------------------------------|---------------------------------------------------|
| **Основной паттерн чтения** | Небольшое количество записей на <br/> один запрос, извлекается по ключу     | Агрегирование по большому количеству записей      |
| **Основной паттерн записи** | Произвольная запись с низким значением <br/> задержки на основе ввода юзера | Групповой импорт данных из другой базы            |
| **В основном применяется**  | Конечные пользователи через веб приложение                                  | Штатные бизнес аналитики                          |
| **Какие данные отражает**   | Актуальное на текущий момент времени                                        | История событий, происходившее на отрезке времени |
| **Размер набора данных**    | От гигабайтов до терабайтов                                                 | От терабайтов до петабайтов                       |

Изначально для OLAP и OLTP использовались одинаковые БД. Позже придумали специализированные БД для OLAP, которые 
назывались складами данных (data warehouse). Склад данных представляет собой отдельную БД, которую аналитики
могут опрашивать так, как им захочется, не влияя при этом на OLTP базу. Склад содержит предназначенные для чтения 
данные из всех различных OLTP баз. Данные извлекаются из OLTP при помощи периодичных выгрузок. Склады так же 
оптимизируются специально с учетом специфики запросов аналитиков. 

Модель склада данных чаще всего реляционная, поскольку SQL в целом отлично подходит для аналитических запросов. 
Множество складов данных имеют схожу структуру, которая называется "схема звезда" (**star scheme**):

![img.png](../../../../img/highload/star_scheme.png)

В центре такой схемы находится таблица фактов. Каждая строка таблицы отражает какое-то событие (покупка, нажатие 
кнопки, звонок, etc). Отдельные столбцы могут представлять собой значение, такие как цена или время. Другие столбцы
могут ссылаться на другие таблицы, именуемые _таблицы измерений_ (dimension table). Каждая из таблиц измерений может
распадаться на другие таблицы измерений.

В типичном складе данных таблицы очень широкие. В них может быть более 100 столбцов, иногда несколько сотен. 

#### Столбцовое хранилище
Если в таблице фактов содержатся триллионы строк, эффективное хранение и выполнение запросов становится нетривиальной 
задачей. Как правило, каждый запрос тянет не все столбцы из таблицы, а несколько из них, а все остальные игнорирует. 
Отсюда пришла идея столбцовых хранилищ: что если хранить данные не построчном в нескольких файлах, а по столбцу в каждом
отдельном файле. Для аналитических запросов это может дать заметный прирост в производительности запросов.

Размещение данных по столбцам требует, чтобы файлы всех столбцов содержали строки в одинаковом порядке. Следовательно 
если необходимо собрать воедино всю строку, надо прочитать значение всех столбцов из всех файлов на одной и той же 
порядковой строке. Вставку в столбцовое хранилище помогает реализовать уже известный нам подход с LSM деревом.

## Резюме
На высоком уровне абстракции все подсистемы хранения делятся на OLTP и OLAP:
1) OLTP-системы нацелены на работу с пользователями, что означает огромное количество запросов, каждый из которых 
затрагивает лишь небольшое число строк. Программы запрашивают строки с помощью ключа, а подсистема хранения задействует
индекс для поиска. 
2) OLAP-системы нацелены на работу с бизнес аналитиками, что означает малое число запросов, но каждый запрос затрагивает
практически всю базу. 

С точки зрения OLTP есть два различных подхода к системам хранения:
1) Журналированный подход, при котором допускает лишь дописывание данных в конец файла, а не обновление записанных 
данных. Раз в какое-то время происходит уплотнение этих файлов. Сюда можно отнести Bitcask, SS-таблицы, LSM-деревья, 
LevelDB, HBase, Lucene и другие.
2) Подход с обновление на месте, при котором диск рассматривается как набор страниц заданного размера, допускающих 
перезапись. Крупнейший представитель здесь - B-деревья, используемые во всех основных реляционных баз данных и многих
нереляционных.
